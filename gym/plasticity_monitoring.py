...
def weight_magnitude(weight_dict):
    squared_weights = [w.pow(2) for w in weight_dict.values()]
    total_squared_value = sum(w.sum() for w in squared_weights)
    total_weight_count = sum(w.numel() for w in squared_weights)

    mean_squared = total_squared_value / (total_weight_count + 1e-8)
    l2_norm = mean_squared.sqrt()

    return round(l2_norm.item(), 4)
...
def gradient_monitoring(results_dir, update, num_updates, flag_grad, grad_):
    grad = grad_.narrow(0, 0, 64).detach().cpu().numpy()
